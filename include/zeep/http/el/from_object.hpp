...
/// \file
/// various implementations of the from_object function that return the data contained in a zeep::http::el::object (http::el::object) object

#include <zeep/config.hpp>

#include <zeep/http/el/factory.hpp>
...
template<typename E>
void from_object(const E& e, std::nullptr_t& n)
{
    if (not e.is_null())
...
template<typename E>
void from_object(const E& e, typename E::boolean_type& b)
{
    if (not e.is_boolean())
...
// template<typename E>
// void from_object(const E& e, typename E::string_type& s)
// {
//     if (not e.is_string())
...
template<typename E>
void from_object(const E& e, std::string& s)
{
    if (not e.is_string())
        throw std::runtime_error("Type should have been string but was " + e.type_name());
    s = *e.template get_ptr<const typename E::string_type*>();
...
template<typename E>
void from_object(const E& e, typename E::int_type& i)
{
    get_number(e, i);
}

template<typename E>
void from_object(const E& e, typename E::float_type& f)
{
    get_number(e, f);
}
...
void from_object(const E& e, A& v)
{
    switch (e.type())
    {
...
void from_object(const E& e, Enum &en)
{
...
template<typename E>
void from_object_array_impl(const E& e, typename E::array_type& arr, priority_tag<3>)
{
    arr = *e.template get_ptr<const typename E::array_type*>();
}

template<typename E, typename T, size_t N>
auto from_object_array_impl(const E& e, std::array<T, N>& arr, priority_tag<2>)
...
template<typename E, typename A>
auto from_object_array_impl(const E& e, A& arr, priority_tag<1>)
    -> decltype(
...
template<typename E, typename A>
void from_object_array_impl(const E& e, A& arr, priority_tag<0>)
{
    std::transform(e.begin(), e.end(), std::inserter(arr, std::end(arr)),
        [](const E& i)
		{
...
void from_object(const E& e, A& arr)
{
    if (not e.is_array())
        throw std::runtime_error("Type should have been array but was " + e.type_name());
    from_object_array_impl(e, arr, priority_tag<3>{});
}

struct from_object_fn
{
    template<typename T>
    auto operator()(const object& j, T& val) const noexcept(noexcept(from_object(j, val)))
    -> decltype(from_object(j, val), void())
    {
        return from_object(j, val);
    }
};

// namespace
// {
//     constexpr const auto& from_object = typename ::zeep::http::el::object::detail::from_object_fn{};
// }
