...
template<typename> struct is_object : std::false_type {};
		...
		is_complete_type_v<std::experimental::detected_t<value_type_t, T>> >>
{
    static constexpr bool value =
		...
		(
			...
			has_from_object_v<typename T::value_type>
		);
};
	...
	std::experimental::is_detected_v<key_type_t, T>>>
{
	...
	static constexpr bool value = 
		...
		(std::is_same_v<typename J::value_type,typename map_t::mapped_type> or
			has_from_object_v<typename T::mapped_type>); 
};
...
struct is_serializable_map_type<T, Archive,
	std::enable_if_t<
		...
		not detail::is_compatible_string_type_v<typename Archive::object_type,T>>>
{
	static constexpr bool value =
