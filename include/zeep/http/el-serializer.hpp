...
struct serializer
{
	...
	struct serializer_impl<T, std::enable_if_t<is_serializable_array_type_v<T,serializer>>>
	{
		static void serialize(const T& data, object_type& e)
		{
			...
			e = object_type::value_type::array;
	...
	struct serializer_impl<T, std::enable_if_t<detail::is_serializable_map_type_v<T,serializer>>>
	{
		static void serialize(const T& data, object_type& e)
		{
			...
			e = object_type::value_type::object;
...
struct deserializer
{
	...
	deserializer(const object_type& elem) : m_elem(elem) {}
...
{
	template<typename T>
	auto operator()(http::object& j, T&& val) const noexcept(noexcept(to_object(j, std::forward<T>(val))))
