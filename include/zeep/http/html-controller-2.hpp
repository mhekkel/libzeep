...
class html_controller_2 : public controller
{
  public:
	html_controller_2(const std::string &prefix_path = "/")
  ...
  public:
	...
	struct mount_point_base
	{
		...
		virtual reply call(const parameter_pack& params, const scope &scope) = 0;
	...
	struct mount_point<Result(ControllerType::*)(const scope &scope, Args...)> : mount_point_base
	{
		...
		mount_point(const char* path, const std::string& method, html_controller_2* owner, Sig sig, Names... names)
		...
		virtual reply call(const parameter_pack& params, const scope &scope)
		{
			try
			{
				...
				invoke<Result>(scope, std::move(args));
			}
