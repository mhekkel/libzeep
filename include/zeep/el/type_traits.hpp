...
/// \file
/// various templated classes that help selecting the right conversion routines when (de-)serializing zeep::json::object (JSON) objects
...
// };
// template <>
// struct is_object<object> : std::true_type
// {
// };
...
// // template <typename T, typename... Args>
// // using from_object_function = decltype(T::from_object(std::declval<Args>()...));

// // template<typename T, typename = void>
// // struct has_to_object : std::false_type {};

// // template<typename T>
// // struct has_to_object<T, std::enable_if_t<not is_object_v<T>>>
// // {
// // 	using serializer = object_serializer<T, void>;
// // 	static constexpr bool value =
// // 		std::experimental::is_detected_exact_v<void, to_object_function, serializer, object&, T>;
// // };
...
// // template<typename T, typename = void>
// // struct has_from_object : std::false_type {};

// // template<typename T>
// // struct has_from_object<T, std::enable_if_t<not is_object_v<T>>>
// // {
// // 	using serializer = object_serializer<T, void>;
// // 	static constexpr bool value =
// // 		std::experimental::is_detected_exact_v<void, from_object_function, serializer, const object&, T&>;
// // };

// // template<typename T>
// // inline constexpr bool has_from_object_v = has_from_object<T>::value;
...
// struct is_object_type<J, T, std::enable_if_t<std::experimental::is_detected_v<mapped_type_t, T> and std::experimental::is_detected_v<key_type_t, T>>>
// {
// 	using map_t = typename J::object_type;
// 	static constexpr bool value =
// 		std::is_constructible_v<std::string, typename map_t::key_type> and
// 		std::is_constructible_v<object, typename map_t::mapped_type>;
// };
...
{
	static constexpr bool value = has_serialize_v<serializer<T>, T>;
};
...
// 		std::is_constructible_v<object, typename T::value_type>>> : std::true_type
// {
// };
...
// 			is_compatible_type_v<typename T::value_type>);
// };
...
// template <typename J, typename T>
// struct is_compatible_object_type<J, T, std::enable_if_t<std::experimental::is_detected_v<mapped_type_t, T> and std::experimental::is_detected_v<key_type_t, T>>>
// {
// 	using map_t = typename J::object_type;
// 	static constexpr bool value =
...
// 			is_compatible_type_v<typename T::mapped_type>);
// };
...
struct is_serializable_map_type<T, Archive,
	std::enable_if_t<
		...
		std::experimental::is_detected_v<iterator_t, T> and
		not is_compatible_string_type_v<typename Archive::object_type, T>>>
{
	static constexpr bool value =
		std::is_same_v<typename T::key_type, std::string> and
		(is_compatible_type_v<typename T::mapped_type> or
...
{
	static constexpr bool value =
		is_compatible_type_v<typename T::value_type> or
