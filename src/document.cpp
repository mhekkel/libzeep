...
char string_data_source::next_byte()
{
	char result = 0;
	if (m_ptr != m_data.end())
	{
...
void document_imp::retract()
{
	...
	m_data.top()->put_back(m_token.back());
	m_token.pop_back();
}
...
int document_imp::get_next_token()
{
	...
	while (token == xml_Undef)
	{
		...
		switch (state)
		{
			...
			case state_TagStart + 20:
				if (is_name_start_char(uc))
					state += 1;
				else
					throw exception("invalid processing instruction target");
				break;
			
			case state_TagStart + 21:
				if (not is_name_char(uc))
				{
					retract();
					if (get_token_string() == "<?xml")
						token = xml_XMLDecl;
					else
						token = xml_PI;
				}
			...
			case state_TagStart + 30:
				if (not is_name_char(uc))
				{
					...
					if (m_token_string == "<!DOCTYPE")
						token = xml_DocType;
					else if (m_token_string == "<!ELEMENT")
						token = xml_Element;
					else if (m_token_string == "<!ATTLIST")
						token = xml_AttList;
					else if (m_token_string == "<!ENTITY")
						token = xml_Entity;
					else if (m_token_string == "<!NOTATION")
						token = xml_Notation;
					else
						throw exception("Unrecognized doctype declaration '%s'", m_token_string.c_str());
				}
				break;
...
void document_imp::xml_decl()
{
	match(xml_XMLDecl);
	match(xml_Space);
	if (m_token_string != "version")
		throw exception("expected a version attribute in XML declaration");
	match(xml_Name);
	eq();
	m_version = boost::lexical_cast<float>(m_token_string);
...
void document_imp::pi()
{
	match(xml_PI);
	while (m_lookahead != xml_Eof and m_lookahead != xml_PIEnd)
		match(m_lookahead);
	match(xml_PIEnd);
}
...
void document_imp::doctypedecl()
{
	...
	if (m_lookahead == '[')
	{
		...
		while (m_lookahead != xml_Eof and m_lookahead != ']')
		{
			switch (m_lookahead)
			{
				case xml_Space:
...
void document_imp::contentspec()
{
	if (m_lookahead == xml_Name)
	{
		if (m_token_string != "EMPTY" and m_token_string != "ANY")
...
document_imp::entity()
{
	...
	if (m_lookahead == '%')	// PEDecl
	{
		...
		{
			value = m_token_string;
			match(xml_String);
		
			resolve_references(value, true);
		}
	...
	else					// GEDecl
	{
		...
		else // ... or an ExternalID
		{
			...
			{
				match(xml_Space);
				if (m_lookahead == xml_String and m_token_string == "NDATA")
				{
					match(xml_String);
					match(xml_Space);
					match(xml_Name);
...
void document_imp::external_id()
{
	if (m_token_string == "SYSTEM")
	{
		match(xml_Name);
		match(xml_Space);
		match(xml_String);
	}
	else if (m_token_string == "PUBLIC")
	{
		match(xml_Name);
		match(xml_Space);
		match(xml_String);
