...
Libzeep comes with one tag_processor implementation, there used to be two, but the legacy one has been removed. The one remaining, :cpp:class:`zeep::http::tag_processor`, is inspired by [@https://www.thymeleaf.org].
