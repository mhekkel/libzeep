...
TEST_CASE("webapp_6")
{
	...
	CHECK(fp1.filename == "1cbs.cif.gz");
	CHECK(fp1.mimetype == "chemical/x-cif");
	CHECK(std::string(fp1.data, fp1.data + fp1.length) == "hello, world!\n");

	auto fp2 = req.get_file_parameter("mtz-file");
	CHECK(fp2.filename == "1cbs_map.mtz");
	CHECK(fp2.mimetype == "text/plain");
	CHECK(std::string(fp2.data, fp2.data + fp2.length) == "And again, hello!\n");
}
...
TEST_CASE("webapp_7")
{
	...
	try
	{
		...
		// reply = zeep::http::simple_request(uri / "XXX");
		// CHECK(reply.get_status() == zh::status_type::bad_request);

		reply = zeep::http::get_request(uri / "test/one");
		CHECK(reply.get_status() == zh::status_type::ok);

		reply = zeep::http::get_request(uri / "test/two");
		CHECK(reply.get_status() == zh::status_type::not_found);
	}
...
TEST_CASE("webapp_8")
{
	...
	try
	{
		...
		reply = zeep::http::send_request(zeep::http::request{ "XXX", uri });
...
TEST_CASE("server_with_security_1")
{
	...
	try
	{
		...
		reply = zeep::http::get_request(uri / "test/three");
...
TEST_CASE("long_filename_test_1")
{
	// start up a http server and stop it again
...
TEST_CASE("pen_test_resilience_1")
{
	// start up a http server and stop it again
